...
In Tenda ac21 V1.0 routers with firmware version V16.03.08.16, the deviceId parameter of route /goform/saveParentControlInfo has a stack overflow vulnerability, which can lead to remote arbitrary code execution.
## Vulnerability Detail
There is a stack overflow vulnerability in the saveParentControlInfo function in Tenda AC21 V1.0 firmware V16.03.08.16. This function accepts the deviceId parameter from a Web request via the variable Var. However, since the user has full control over the input of deviceId, and strcpy() performs the copy operation without any boundary checks to ensure the source string does not exceed the target buffer's size, the statement strcpy(s + 2, Var); leads to a heap-based buffer overflow. The user-supplied deviceId can exceed the bytes allocated for the buffer s, thereby corrupting adjacent memory and metadata on the heap. This vulnerability could be exploited to trigger a Denial of Service (DoS) or achieve Remote Code Execution (RCE) by manipulating heap structures.
![img](./img/saveParentControlInfo.png)
## Poc
```py
