...
There is a stack overflow vulnerability in the sub_42DA54 function in DLink DIR816 firmware 1.10CNB05. The function sub_42DA54, which is registered to handle the "dir_setWanWifi" web form, accepts the statuscheckpppoeuser parameter from a Web request via the variable v6. However, since the user has full control over this input and the data is directly concatenated into a shell command string, the statement sprintf(v32, "echo %s >/tmp/statuscheckpppoeuser", v6); followed by system(v32); leads to an OS Command Injection vulnerability. An attacker can provide a specially crafted value for statuscheckpppoeuser containing shell metacharacters (such as ;, &, |, or `) to execute arbitrary commands on the system with the privileges of the web server.
