...
In DLink DIR816 routers with firmware version 1.10CNB05, the s_mac parameter of route /goform/addassignment has a stack overflow vulnerability, which can lead to remote arbitrary code execution.
## Vulnerability Detail
There is a stack overflow vulnerability in the sub_42BC84 function in DLink DIR816 firmware 1.10CNB05. The function sub_42BC84, which is registered to handle the "addassignment" web form, accepts the s_mac parameter from a Web request via the variable v3. However, since the user has full control over the input and the strcat() function performs concatenation without any boundary checks to ensure the total string length does not exceed the target buffer's size, the statement strcat(v10, v3); leads to a stack-based buffer overflow. The cumulative length of the data already stored in the 1024-byte buffer v10 (retrieved from NVRAM) combined with the user-supplied MAC address can easily exceed the array's capacity, overwriting the stack frame and the saved return address. This vulnerability can be exploited to achieve Remote Code Execution (RCE) or trigger a Denial of Service (DoS).
![img](./img/addassignment-mac.png)
...
Cookie: password=708a9c84b47404c5524405e5cbd910b8qwq5gk; uid=JpF2tEMYvf
